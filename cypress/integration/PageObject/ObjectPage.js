class SigninPage {
    ...
    checkErrorMessage(errormessage,color,background) {
    ...
    checkErrorIcon(color,background) {
        cy.get('.MuiAlert-icon > .MuiSvgIcon-root > path')
        .should('be.visible')
        .should('have.css', 'color', color)
        .should('have.css', 'background-color', background);
        return this
    ...
    allFieldsCheck(text1,text2) {
        cy.get('fieldset.PrivateNotchedOutline-root-7.MuiOutlinedInput-notchedOutline')
        .should('be.visible')
        .should('contain', text1)
        .should('contain', text2)
        return this
    }
    usernameFieldErrorColorCheck(text,color) {
        cy.get('#username-label')
        .should('contain', text)
        .should('have.css', 'color',color)
        return this
    }
    passwordFieldErrorColorCheck(text,color) {
    ...
    checkErrorMessageAndColor (text,color) {
        cy.get('#username-helper-text')
        .should('contain', text)
        .should('have.css', 'color', color)
    }
    checkErrorMessageAndColor () {
    ...
    checkHeader(text,color) {
        cy.get('.MuiTypography-h5')
        .should('be.visible')
        .should('contain', text)
        .should('have.css', 'color', color)
    }
    checkRememberMeTitle(text,color) {
        cy.get('.MuiFormControlLabel-label.MuiTypography-body1')
        .should('be.visible')
        .should('contain', text)
        .should('have.css', 'color', color)
    }
    checkDontHaveAnAccountLink(color,text) {
    ...
    checkBuiltByTitle(color,text) {
        cy.get('.MuiTypography-colorTextSecondary.MuiTypography-alignCenter')
        .should('be.visible')
        .should('have.css', 'color', color)
        .and('contain',text)

    }
    checkLogo1Logo2Color(color) {
        cy.get('.makeStyles-logo-3')
        .should('have.css', 'color', color)
    }
    checkCypressIoLink() {
        cy.get('.MuiTypography-colorTextSecondary.MuiTypography-alignCenter > a')
